...
namespace fnv1a {
    ...
    __forceinline hash_t fnv_hash(const char* str) {
    ...
    __forceinline constexpr hash_t fnv_hash_const(const char(&str)[N], const std::size_t i = N) {
        return i == 1 ? (FNV_OFFSET_BASIS ^ str[0]) * FNV_PRIME : (fnv_hash_const(str, i - 1) ^ str[i - 1]) * FNV_PRIME;
    }

    template <class T>
    __forceinline constexpr hash_t fnv_hash_type() {
#ifdef _MSC_VER
