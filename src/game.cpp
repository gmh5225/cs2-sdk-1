...
namespace game {
    sdk::CSPlayerController* get_entity_index(const std::int32_t index) {
        ...
        static auto fn = memory::find_pattern(L"client.dll", "E8 ? ? ? ? 4C 3B E0").jmp().as<Fn>();
    ...
    sdk::CSPlayerController* get_player_controller(const std::int32_t index) {
        ...
        static auto fn = memory::find_pattern(L"client.dll", "40 53 48 83 EC ? 48 8B 05 ? ? ? ? 48 85 C0 74 3D").as<Fn>();
    ...
    sdk::CSPlayerPawn* get_player_pawn(sdk::CSPlayerController* controller) {
        ...
        static auto fn = memory::find_pattern(L"client.dll", "E8 ? ? ? ? 48 85 C0 74 E2").jmp().as<Fn>();
