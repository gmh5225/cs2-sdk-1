...
namespace pe {
    ...
    std::optional<ModuleInfo> get_module_by_name(const std::wstring_view module_name) {
        static const auto modules = get_loaded_modules();

        if (!modules)
            return std::nullopt;

        const auto it = std::ranges::find_if(*modules, [&module_name](const ModuleInfo& module) {
